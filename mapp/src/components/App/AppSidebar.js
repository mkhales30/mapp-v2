...
// This is the sidebar with courses
function AppSidebar({courses, toggleModal, updateCourse}) {
    // Get the currently selected course from the store
    const selectedCourse = useSelector((state) => state.selectedCourse.value)
    // The dispatch function wll be used to trigger actions to the store
        ...
        <div
            ...
            */}
            ...
            <div
                ...
                {/*Add Course Button*/}
                <button className='hover:text-green-500 flex flex-row gap-2 text-gray-400 opacity-50 items-center'
                        onClick={toggleModal}>
            ...
            {courses.length > 0 ? (
                courses.map((course, index) => {
                    return (
                        <Link
                            // When a course is clicked, update the selected course in the store
