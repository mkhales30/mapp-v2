...
function ProfilePictureUploadModal({ isOpen, onClose, onUpload }) {
    ...
    const handleSubmit = async () => {
        ...
        if (userDocSnapshot.exists()) {
            ...
            }, { merge: true }); // Use merge option to preserve existing fields
        } else {
                ...
                <div className='h-full w-full top-0 left-0 right-0 bottom-0 fixed z-30'>
                    ...
                    <div className="absolute grid h-screen w-screen place-items-center">
                        <div className="relative flex flex-col gap-4 bg-white min-w-[500px] max-h-[500px] p-8 rounded">
                            ...
                            <button>
                                <FontAwesomeIcon className={" absolute top-2 right-2 w-3 h-3"} onClick={onClose} icon={faTimes}/>
                            </button>
                            <input type="file" onChange={handleFileChange} />
                            <button className='bg-stone-800 text-white text-center px-4 py-2 w-full rounded text-lg' onClick={handleSubmit}>
                                Upload
                            </button>
                        </div>
                    </div>
